...
- **Telegram 话题 (Thread/Topic) 支持**：支持为指定群组的话题 ID 发送签到消息，补全了复杂的 Telegram 论坛交流群的自动签到需求。
- **全局代理回退机制**：新增全局代理设置。如果个别账号没有显式配置独立代理，系统将自动回退使用全局代理。
- **剪贴板批量导入导出**：在主全局任务列表页面新增批量导出配置到剪贴板、以及从剪贴板一键智能导入防重复的功能，彻底解决跨服务器快速迁移任务的需求。
- **便捷重登体验**：首页点击进入特定账号编辑界面后，新增独立的一键“重新登录”按钮。解决账号意外过期状态无法显式重连的问题，优化操作手感。
