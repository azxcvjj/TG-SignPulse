...
const loadChats = async (n: string) => { const t=localStorage.getItem('tg-signer-token')||''; try{availableChats.value=await getAccountChats(t,n)}catch(e){console.error(e)} }
...
watch(selectedAccount,(v)=>{if(v)loadChats(v)})
let st:any=null
...
return{name:taskName.value||undefined,account_name:selectedAccounts.value[0]||'',account_names:selectedAccounts.value,sign_at:sa,execution_mode:em,range_start:rs,range_end:re,random_seconds:0,chats:[{chat_id:selectedChatId.value,name:selectedChatName.value,actions:ca,message_thread_id:messageThreadId.value?Number(messageThreadId.value):undefined}]}}
  ...
  <div class="space-y-6 text-left">
    ...
    <div class="p-4 border border-sky-100 dark:border-gray-800/60 bg-sky-50/50 dark:bg-gray-900/40">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="space-y-1.5"><label class="text-xs font-medium text-gray-500">{{ t('taskForm.selectFromList') }}</label><CustomSelect v-model="selectedChatId" :options="[{label: t('taskForm.selectChat'), value:0}, ...availableChats.map(c => ({label: c.title || c.username || c.id, value: c.id}))]" @update:modelValue="selectedChatName = availableChats.find(c => c.id === $event)?.title || String($event)" /></div>
